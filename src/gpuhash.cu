...
__global__ void search_sha512_kernel(const __restrict uint64_t *dev_data, __restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits);
__global__ void filter_sha512_kernel(__restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits);
__global__ void filter_and_rewrite_sha512_kernel(__restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits, __restrict uint64_t *dev_results);
__global__ void populate_filter_kernel(__restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits);
...
__device__
void set_or_double(__restrict uint32_t *countbits, uint32_t whichbit) {
...
__device__ inline
void add_to_filter(__restrict uint32_t *countbits, const uint64_t hash) {
  uint32_t whichbit = (uint32_t(hash>>14) & ((1UL<<COUNTBITS_SLOTS_POWER)-1));
  set_or_double(countbits, whichbit);
}

__device__ inline
bool is_in_filter_twice(__restrict uint32_t *countbits, const uint64_t hash) {
  uint32_t whichbit = (uint32_t(hash>>14) & ((1UL<<COUNTBITS_SLOTS_POWER)-1));
...
__global__
void search_sha512_kernel(const __restrict uint64_t *dev_data, __restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits) {
  uint64_t H[8];
  uint64_t D[5];
...
__global__
void filter_sha512_kernel(__restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits) {
  uint32_t spot = (((gridDim.x * blockIdx.y) + blockIdx.x)* blockDim.x) + threadIdx.x;
  for (int i = 0; i < 8; i++) {
...
__global__
void populate_filter_kernel(__restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits) {
  uint32_t spot = (((gridDim.x * blockIdx.y) + blockIdx.x)* blockDim.x) + threadIdx.x;
  for (int i = 0; i < 8; i++) {
    uint64_t myword = dev_hashes[i*POOLSIZE+spot];
    if (myword) {
...
__global__
void filter_and_rewrite_sha512_kernel(__restrict uint64_t *dev_hashes, __restrict uint32_t *dev_countbits, __restrict uint64_t *dev_results) {
  uint32_t spot = (((gridDim.x * blockIdx.y) + blockIdx.x)* blockDim.x) + threadIdx.x;
  for (int i = 0; i < 8; i++) {
    uint64_t myword = dev_hashes[i*POOLSIZE+spot];
